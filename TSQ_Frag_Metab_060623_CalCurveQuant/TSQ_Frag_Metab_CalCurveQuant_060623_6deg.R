...
norm_peaks <- function(compound_name){
  ...
  CalCurve_cals[paste("Heavy", compound_name, "Norm", sep = ".")] <- CalCurve_cals$Total.Area /CalCurve_cals[paste("Heavy", compound_name, "Peaks", sep = ".")]
...
# Apply function over all 4 compounds
# Note: use lapply instead
norm_peaks("B1")
norm_peaks("B2")
norm_peaks("B12-CN")
norm_peaks("B7")
...
# Select BMIS and produce final peak to be used for plotting and quantification  -------------------------------------------------------------
...
BMIS.Data <- BMIS.Data %>% dplyr::select(Molecule.Name, BMIS_used)

# Join BMIS and calibration curve data
CalCurve_BMIS <- left_join(CalCurve_cals, BMIS.Data, by = "Molecule.Name")

# Select only relevant columns
CalCurve_BMIS <-
  CalCurve_BMIS %>% dplyr::select(
    Replicate.Name,
    Molecule.Name,
    Total.Area,
    Heavy.B1.Norm,
  ...
  rename(Heavy.CNB12.Norm = `Heavy.B12-CN.Norm`)

# Generate final peaks based on BMIS used 
# Will use "matching compounds" (ex: B12's to CNB12, B1 to heavy B1, B2 to heavy B2). # B7 was not matched to heavy B7 due to an increase in cv
# For others, will use a compound as BMIS if improves cv by >30%
# For each line, look at BMIS used. If none, make final peak the unnormalized one
# If not none, match BMIS to the heavy column and use this value
CalCurve_BMIS$BMIS_col_name <- paste("Heavy.", CalCurve_BMIS$BMIS_used,".Norm", sep = "")

# Remove heavy entries
CalCurve_BMIS <- dplyr::filter(CalCurve_BMIS, !grepl("heavy",Replicate.Name)) 
...
# Narrow dataframes to only most dilute points in calibration curves
LODQ_df <- CalCurve_BMIS %>% filter( grepl("p5",Replicate.Name)) %>%
  group_by(Molecule.Name) %>%
  dplyr::summarise(sd_peak = sd(Final_Peak), LOD = 3*sd(Final_Peak), LOQ = 5* sd(Final_Peak))
...
# Remove entries that don't have cal 
...
cal_spike <- left_join(CalCurve_BMIS, spike_amounts, by = "Replicate.Name") %>% inner_join(cal_data_sum, by = "Molecule.Name")

# Add a blank column for corrected spike amounts 
cal_spike$spike_amount_corr <- NULL
